...
(defn get-user-input [player map-stack loc-info enter? exit? loot? n s e w]
  (let [input (read-line)]
    (cond
      ...
      (= input "l") (if loot?
                      (let [[new-player new-loc-info] (find-loot player map-stack loc-info)]
      ...
      (= input "i") (let [[new-player new-loc-info] (access-inv player map-stack loc-info)]
                      [new-player map-stack new-loc-info])
...
(defn is-there-loot? [player map-stack loc-info]
  (if (empty? (get-loc-loot player map-stack loc-info))
  ...
  (let [player-loc-name (get-player-loc player map-stack)]
    (let [get-loot-from (get-in loc-info [player-loc-name :get-loot-from])]
      (if (nil? get-loot-from)
        ...
        ;FIXME - shuffle needs to be in a monad
        (do
        (let [half-updated-loc-info (assoc-in loc-info [player-loc-name :loot] (take 2 (shuffle (get-loot-from loot-types))))]
